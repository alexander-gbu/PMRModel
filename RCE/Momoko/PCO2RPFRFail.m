...
p.D0co = 1e-1; %guessed
p.D0coliq = 1e3;
p.K = 0.01; % = kG*kL/(H*kL+kG) in cm/min mass transfer coeffienct for co in water 0.14 mm/s
p.v0 = p.F/p.SAliq;
...
title(['CO concentration in liquid at ', num2str(p.P), ' bar'])
...
function [c,f,s] = pde(x,t,u,dudx,p)
    c = [1; 1];
    f = [p.D0coliq*dudx(1); -p.v0*0.1*u(2)];
    s = [p.R; 0];
end
